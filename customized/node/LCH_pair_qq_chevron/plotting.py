"""Single-excitation flux-chevron visualization: one figure per qubit pair.

With state discrimination the probe saves the joint two-qubit populations
P00/P01/P10/P11 (vars `state_gg/state_ge/state_eg/state_ee`, first digit = control,
second = target); each pair gets a figure with four color-map panels (00, 01, 10, 11)
over the flux-pulse amplitude (y) x duration (x) sweep. Without state discrimination
the raw I quadratures are shown instead (control | target).
"""
...
def plot_chevron_2d(ds_raw: xr.Dataset, qubit_pairs, *, use_state_discrimination: bool) -> Dict[str, plt.Figure]:
    ...
    for pair in pair_names:
        ...
        for col, (var, label) in enumerate(panels):
            ax = axs[0, col]
            ds_raw[var].sel(qubit_pair=pair).plot(x="time", y="amplitude", ax=ax, add_colorbar=True)
            ax.set_title(title_for(label))
            ax.set_xlabel("Pulse duration [ns]")
            ax.set_ylabel("Flux amplitude prefactor")
        fig.suptitle(f"Single-excitation flux chevron - {pair}")
        fig.tight_layout()
        figures[pair] = fig
