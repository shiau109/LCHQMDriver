...
from calibration_utils.LCH_CZ_conditional_phase_fast2D import (
    Parameters,
    fit_raw_data,
    log_fitted_results,
...
node = QualibrationNode[Parameters, Quam](
    name="LCH_CZ_conditional_phase_fast2D",
...
def create_qua_program(node: QualibrationNode[Parameters, Quam]):
    ...
    # Coupler Flux amp sweep in V
    num = node.parameters.num_coupler_amp_points
    coupler_amps = np.linspace(node.parameters.min_coupler_amp_in_v , node.parameters.max_coupler_amp_in_v, num)
    # Qubit Flux amp sweep in V
    num = node.parameters.num_qubit_amp_points
    qubit_amps = np.linspace(node.parameters.min_qubit_amp_in_v , node.parameters.max_qubit_amp_in_v, num)


    flux_idle_case = node.parameters.flux_idle_case

    ctrl_switch = [True, False]  # Control of CZ
    readout_basis_array = [0,1]
    ...
    node.namespace["sweep_axes"] = {
        ...
        "basis": xr.DataArray(np.array(readout_basis_array), attrs={"long_name": "control switch", "units": "arb."}),
    }
    ...
    with program() as node.namespace["qua_program"]:
        ...
        rbi = declare(int)  # QUA variable for readout basis index
        ...
        with stream_processing():
            ...
            for i in range(num_qubits):
                if node.parameters.use_state_discrimination:
                    state_st[i].buffer(len(readout_basis_array)).buffer(len(ctrl_switch)).buffer(len(coupler_amps)).buffer(len(qubit_amps)).average().save(f"state{i + 1}")
                else:
                    I_st[i].buffer(len(readout_basis_array)).buffer(len(ctrl_switch)).buffer(len(coupler_amps)).buffer(len(qubit_amps)).average().save(f"I{i + 1}")
                    Q_st[i].buffer(len(readout_basis_array)).buffer(len(ctrl_switch)).buffer(len(coupler_amps)).buffer(len(qubit_amps)).average().save(f"Q{i + 1}")
