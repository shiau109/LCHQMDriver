...
import numpy as np
import xarray as xr
from customized.node.LCH_iswap_fixed_time import (
    Parameters,
    baked_waveform,
    fit_raw_data,
    log_fitted_results,
...
node = QualibrationNode[Parameters, Quam](
    name="LCH_iswap_fixed_time",  # Name should be unique
...
def create_qua_program(node: QualibrationNode[Parameters, Quam]):
    ...
    # Loop parameters
    ctrl_amps = np.arange( node.parameters.ctrl_amp_min, node.parameters.ctrl_amp_max, node.parameters.ctrl_amp_step)
    cplr_amps = np.arange( node.parameters.cplr_amp_min, node.parameters.cplr_amp_max, node.parameters.cplr_amp_step)

    clock_cycles = np.arange(4, node.parameters.max_time_in_ns//4)
    times_cycles = clock_cycles *4

    node.namespace["sweep_axes"] = {
        "qubit_pair": xr.DataArray(qubit_pairs.get_names()),
        "ctrl_amps": xr.DataArray(ctrl_amps, attrs={"long_name": "amplitudes of the ctrl flux pulse"}),
        "cplr_amps": xr.DataArray(cplr_amps, attrs={"long_name": "amplitudes of the cplr flux pulse"}),
    }
    ...
    with program() as node.namespace["qua_program"]:
        ...
        ctrl_a = declare(fixed)
        cplr_a = declare(fixed)
        n = declare(int)
        n_st = declare_stream()
        ...
        for multiplexed_qubit_pairs in qubit_pairs.batch():
            ...
            with for_(n, 0, n < n_avg, n + 1):
                ...
                with for_(*from_array(ctrl_a, ctrl_amps)):
                    ################################################################################################
                    ...
                    ################################################################################################
                    with for_(*from_array(cplr_a, cplr_amps)):
                        for ii, qp in multiplexed_qubit_pairs.items():
                            ...
                            qp.macros[operation].apply(ctrl_amp=ctrl_a, cplr_amp=cplr_a)
        ...
        with stream_processing():
            ...
            for i in range(num_qubit_pairs):
                if node.parameters.use_state_discrimination:
                    state_c_st[i].buffer(len(cplr_amps)).buffer(len(ctrl_amps)).average().save(f"state_control{i}")
                    state_t_st[i].buffer(len(cplr_amps)).buffer(len(ctrl_amps)).average().save(f"state_target{i}")
                else:
                    I_c_st[i].buffer(len(cplr_amps)).buffer(len(ctrl_amps)).average().save(f"I_control{i}")
                    Q_c_st[i].buffer(len(cplr_amps)).buffer(len(ctrl_amps)).average().save(f"Q_control{i}")
                    I_t_st[i].buffer(len(cplr_amps)).buffer(len(ctrl_amps)).average().save(f"I_target{i}")
                    Q_t_st[i].buffer(len(cplr_amps)).buffer(len(ctrl_amps)).average().save(f"Q_target{i}")
